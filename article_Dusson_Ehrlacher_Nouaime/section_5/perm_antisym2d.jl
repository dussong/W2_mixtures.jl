...
M2 = mixture(3, [0.4; 0.3; 0.3], [A2; A3; A5])
...
for i in 1:nb_images
   for j in 1:nb_images
      ...
      push!(barmw2, contour(X, X, density.(Ref(bt), XX), c=:viridis, aspect_ratio=:equal, clim=(0.0, 10.), fill=:false, yformatter=_ -> "", xformatter=_ -> "", colorbar=:false))

      println("Contour ($i, $j) done.")
   end
end

pbarmw2 = plot(barmw2..., size = nb_images .* (512, 512), layout = grid(nb_images, nb_images))
savefig(pbarmw2, "data_article_Dusson_Ehrlacher_Nouaime/section5_permantisym/bary_mw2.pdf")
...
a2dens = 1e-16 .+ density.(Ref(M2), XY)
a3dens = 1e-16 .+ density.(Ref(M3), XY)

A = zeros(length(a0dens), 4)
A[:, 1] = a0dens
A[:, 2] = a1dens
A[:, 3] = a2dens
A[:, 4] = a3dens
...
for i in 1:nb_images
   for j in 1:nb_images
      ...
      λ = (1 - ty) * tmp1 + ty * tmp2
      @show λ
      bt = ot.barycenter(A, M, reg, λ, method="sinkhorn_log", numItermax=10000)
      P = contour(X, X, (reshape(bt, nx, nx))', c=:viridis, aspect_ratio=:equal, clim=(0.0, 10.), fill=:false, yformatter=_ -> "", xformatter=_ -> "", colorbar=:false)
      display(P)

      push!(barw2, contour(X, X, (reshape(bt, nx, nx))', c=:viridis, aspect_ratio=:equal, clim=(0.0, 10.), fill=:false, yformatter=_ -> "", xformatter=_ -> "", colorbar=:false))

      println("Contour ($i, $j) done.")
   end
end

pbarw2 = plot(barw2..., size = nb_images .* (512, 512), layout = grid(nb_images, nb_images))
savefig(pbarw2, "data_article_Dusson_Ehrlacher_Nouaime/section5_permantisym/bary_w2.pdf")
